...
const FeedbackComponent = () => {
  ...
  const avaliacao = [joaoNota, diegoNota, anaNota];

  return (
    <Element name="Depoimentos" className="p-8 sm:p-0">
      <div className=" sm:mx-[200px] sm:mt-12 py-12">
        <div className="flex gap-2" id="pai">
          {/* IMAGEM DA PIZZA */}
          <div
            className="sm:flex-1 relative p-[10px] hidden sm:block"
            id="filho1"
          >
            <img src="assets/pizzaBnr.svg" width={570} />
          </div>
          {/* O QUE DIZEM SOBRE NÓS */}
          <div className="flex-1 p-[10px]  sm:w-auto w-3" id="filho2">
            <b className="text-colorPrimary  text-base uppercase tracking-[5px] flex justify-center ">
              Feedback
            </b>
            ...
            <div className="flex gap-3">
              {clientes.map((nd, i) => (
                <button
                  key={i}
                  className={`py-2 px-4 bg-[#f5f5f5] shadow-xl rounded-xl flex items-center ${
                    botaoAtivo === i
                      ? "bg-colorSecondary"
